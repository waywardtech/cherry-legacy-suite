...
function check_widget_visibility($id) {
	...
	if ( isset($options[$id]) && $options[$id] != '' ) {
		// build condition expression and evaluate it (PHP 7/8 compatible)
		$expr = cherry_condition($options_type[$id], $options[$id], $users[$id]);
		if ( ! empty( $expr ) ) {
			$conditional_result = (bool) eval( 'return ' . $expr . ';' );
		} else {
			$conditional_result = false;
		}
