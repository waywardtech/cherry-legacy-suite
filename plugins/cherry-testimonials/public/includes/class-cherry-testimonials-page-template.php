...
class Cherry_Testimonials_Page_Template {
	...
	public function __construct() {
		...
		// Defer template-label translation to `init` (priority 20) so it runs
		// AFTER the plugin textdomain is loaded by Cherry_Testimonials::lang()
		// at init:1. Calling __() at plugin-load time triggers a WP 6.7+
		// _load_textdomain_just_in_time notice. The former trailing lines
		// building a local $templates array from wp_get_theme() were unused
		// and have been dropped.
		add_action( 'init', array( $this, 'register_template_labels' ), 20 );
	...
	/**
	 * Populate the tracked page templates with their translated labels.
	 *
	 * Hooked on `init` (priority 20) so the plugin textdomain is available
	 * before __() runs (WP 6.7+).
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function register_template_labels() {
		$this->templates = array(
