...
if ( !class_exists( 'Cherry_Api_Js' ) ) {
	class Cherry_Api_Js {
		...
		public function localize_script() {
			...
			// WP 5.7+ _doing_it_wrong: wp_localize_script()'s $l10n parameter must be
			// an array. The old code passed a raw nonce string, relying on an undocumented
			// fallback. wp_add_inline_script() is the API WP explicitly recommends for
			// scalar/arbitrary-data injection, and with wp_json_encode() it produces the
			// same frontend contract (`var cherry_ajax = "<nonce>";`) so consumer JS is
			// unchanged.
			wp_add_inline_script(
				'cherry-api',
				'var cherry_ajax = ' . wp_json_encode( wp_create_nonce( 'cherry_ajax_nonce' ) ) . ';',
				'before'
			);
		}
	...
	$Cherry_Api_Js = new Cherry_Api_Js( array( 'product_type' => 'plugin' ) );
}