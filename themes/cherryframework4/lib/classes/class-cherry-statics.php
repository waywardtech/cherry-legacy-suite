...
class Cherry_Statics {
	...
	public static function register_static( $args = array() ) {
		...
		if ( ! isset( $cherry_registered_statics[ $id ] ) ) {
			...
			// PHP 8.2+ deprecates the literal string 'self' in callables (error in PHP 9).
			// __CLASS__ is the semantic equivalent: compile-time-resolved class name,
			// no late static binding needed since Cherry_Statics::compare() isn't overridden.
			uasort( $cherry_registered_statics, array( __CLASS__, 'compare' ) );
		}
	}
	...
	public static function static_area( $index = 1 ) {
		...
		foreach ( $args as $id => $data ) :
			...
			// PHP 8.2+ deprecates the literal string 'self' in callables (see uasort
			// above). __CLASS__ is the semantic equivalent for this non-overridden
			// static method.
			array_walk( $cols, array( __CLASS__, 'prepare_column_class' ), $options );
