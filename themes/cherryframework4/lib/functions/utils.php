 ...
 * @return double            Size.
 */
// PHP 8.0+ deprecates optional-before-required parameters. $percent has no
// default, so $operation and $func were already implicitly required. Drop
// their defaults to align the signature with what PHP already enforces.
// (This function is only referenced as a string callback via the CSS parser
// map; any caller using it must already be passing all four args.)
function cherry_typography_size( $size, $operation, $func, $percent ) {
